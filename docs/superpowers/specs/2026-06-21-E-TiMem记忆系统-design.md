...
| ID | 层级 | 粒度 | 存储 | 整合方式 | 预估 |
|----|------|------|------|----------|------|
| E1 | L₁ 片段层 | 单轮对话 | Redis Hash | 在线同步整合 | 2h |
...
Value: JSON { user_id, session_id, created_at }

Key:   memory:queue:L3     # L3 整合任务队列 (List)  
Value: JSON { user_id, date, created_at }

# Worker 消费:
#   BRPOP memory:queue:L2 → 执行整合 → 写入 PG → ACK
#   BRPOP memory:queue:L3 → 执行整合 → 写入 PG → ACK
```
...
```python
# 双通道评分融合
score = 0.9 * cosine_similarity(query_emb, memory_emb) + 0.1 * bm25_score(query_text, memory_text)
...
| `backend/app/sse/stream.py` | 修改 | 添加 chat.recall 事件 + L1 在线整合 |
| `backend/app/api/v1/recommend.py` | 修改 | 会话结束时入队 L2 任务 |
...
├── SSE: chat.recall ──────── 规划器 → 检索 → 门控 (50ms)
...
│
│  用户关闭/重置会话
│  [L2 入队] ───────────────── RPUSH memory:queue:L2 {user_id, session_id}
│
│  Worker (后台进程):
...
| 操作 | 预估延迟 | 说明 |
|------|----------|------|
| L1 片段写入 | < 2ms | Redis SET |
| L1 片段整合 (LLM) | ~500ms | 1 次 LLM，简短 Prompt |
...
| 分层检索 (向量) | ~10ms | pgvector cosine + BM25 |
| 召回门控 (LLM) | ~300ms | 过滤+排序，短回答 |
| 召回总量 | ~500ms | 规划器 200ms + 检索 10ms + 门控 300ms |
| **用户感知增加** | **~500ms** | chat.recall 新事件，在 skeleton 之前 |
